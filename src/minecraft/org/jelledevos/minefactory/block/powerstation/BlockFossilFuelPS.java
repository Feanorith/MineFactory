package org.jelledevos.minefactory.block.powerstation;
...
public class BlockFossilFuelPS extends BlockContainer implements IElectricalUnit, IElectricityProducer {
	
    /**	Is the random generator used to drop the inventory contents in random directions upon destruction. */
    private Random dropRand = new Random();
    
    /**	True if this is an active PS, false if idle. */
    private final boolean isActive;
    
    /**	Determines whether to drop inventory upon block removal. */
    private static boolean keepInventory = false;

    /**
     *	The ONLY constructor the this block.
     *	@param blockID = The block id this block will be registered under.
     *	@param isActive = Whether or not this block is an active PS, or an idle one.
     */
	public BlockFossilFuelPS(int blockID, boolean isActive)
	{
		super(blockID, Material.rock);
		this.isActive = isActive;
		
	    blockIndexInTexture = 45;
	    setBlockName("fossilFuelPowerStation");
	    setHardness(0.5F);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int direction, float xOffset, float yOffset, float zOffset) {
        if (world.isRemote) {
            ...
            TileEntityFossilFuelPS entity = (TileEntityFossilFuelPS)world.getBlockTileEntity(x, y, z);
            if (entity != null)
    ...
    public int idDropped(int metadata, Random random, int fortune) {
        return MineFactory.instance.blockFossilFuelPSIdle.blockID;
    }
    ...
    public int getBlockTexture(IBlockAccess blockAccess, int x, int y, int z, int side) {
        if (side == 1) {
            ...
            return side != blockMetadata ? this.blockIndexInTexture : (this.isActive ? this.blockIndexInTexture + 16 : this.blockIndexInTexture - 1);
        }
    }
    
    /**	Creates a TileEntity for the block. */
    public TileEntity createNewTileEntity(World world) {
        return new TileEntityFossilFuelPS();
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLiving entity)
    {
        ...
        TileEntityFossilFuelPS tileEntity = (TileEntityFossilFuelPS) world.getBlockTileEntity(x, y, z);
        tileEntity.setConnectedElectricityReceivers(ElectricConnectionManager.getEnergyDestinations(world, x, y, z));
        
    }
    
    /**
     *	Update which block ID the PS is using depending on whether or not it isActive.
     *	@param isActive = Whether or not the PS is active. (Burning fuel.)
     * 	@param world = The world object.
     *  @param x = The block's x position.
     *	@param y = The block's y position.
     *	@param z = The block's z position.
     */
    public static void updatePSBlockState(boolean isActive, World world, int x, int y, int z) {
        int metadata = world.getBlockMetadata(x, y, z);
        TileEntity tileEntity = world.getBlockTileEntity(x, y, z);
        keepInventory = true;

        if (isActive) {
            world.setBlockWithNotify(x, y, z, MineFactory.instance.blockFossilFuelPSActive.blockID);
        } else {
            world.setBlockWithNotify(x, y, z, MineFactory.instance.blockFossilFuelPSIdle.blockID);
        }
        ...
        if (!keepInventory) {
            ...
            if (tileEntity != null) {
                for (int i = 0; i < tileEntity.getSizeInventory(); ++i) {
                    ...
                    if (itemStack != null) {
                        float motionX = this.dropRand.nextFloat() * 0.8F + 0.1F;
                        float motionY = this.dropRand.nextFloat() * 0.8F + 0.1F;
                        float motionZ = this.dropRand.nextFloat() * 0.8F + 0.1F;

                        while (itemStack.stackSize > 0) {
                            int randomInt = this.dropRand.nextInt(21) + 10;

                            if (randomInt > itemStack.stackSize)
                                randomInt = itemStack.stackSize;

                            itemStack.stackSize -= randomInt;
                            EntityItem entityItem = new EntityItem(world, (double)((float)x + motionX), (double)((float)y + motionY), (double)((float)z + motionZ), new ItemStack(itemStack.itemID, randomInt, itemStack.getItemDamage()));

                            if (itemStack.hasTagCompound())
                                entityItem.item.setTagCompound((NBTTagCompound)itemStack.getTagCompound().copy());

                            float motion = 0.05F;
                            entityItem.motionX = (double)((float)this.dropRand.nextGaussian() * motion);
                            entityItem.motionY = (double)((float)this.dropRand.nextGaussian() * motion + 0.2F);
                            entityItem.motionZ = (double)((float)this.dropRand.nextGaussian() * motion);
	...
	public void updateValidReceivers(World world, int x, int y, int z) {
        TileEntityFossilFuelPS tileEntity = (TileEntityFossilFuelPS) world.getBlockTileEntity(x, y, z);
        tileEntity.updateValidReceivers(world, x, y, z);
	}
